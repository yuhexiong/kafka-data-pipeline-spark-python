...
for field in fields:
    if field['from_type'] == 'array' or field['from_type'] == 'list':
        ...
        sub_fields = [StructField(sub_field['name'], get_spark_type(sub_field['from_type'])) for sub_field in sub_fields]
        schema_fields.append(StructField(field['name'], ArrayType(StructType(sub_fields))))
    else:
        schema_fields.append(StructField(field['name'], get_spark_type(field['from_type'])))
...
for field in fields:
    if field['from_type'] == 'array' or field['from_type'] == 'list':
        ...
        for sub_field in sub_fields:
            column = get_selected_column(f"{field['name']}.{sub_field['name']}", sub_field['alias'], sub_field['from_type'], sub_field['to_type'])
            selected_columns.append(column)
    else:
        column = get_selected_column(f"{field['name']}", field['alias'], field['from_type'], field['to_type'])
        selected_columns.append(column)
...
for step in process['steps']:
    if step['operation'] == "select_expr":
        ...
        processed_df = processed_df.select(from_json(col("json_string"), value_schema).alias(step['value']))
    elif step['operation'] == "select_column":
        processed_df = processed_df.select(step['value'])
    elif step['operation'] == "explode_column":
        processed_df = processed_df.withColumn(step['value'], explode(col(step['value'])))
